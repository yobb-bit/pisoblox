...
import { X, Zap, CheckCircle, Upload, Hash } from "lucide-react";
...
export function PaymentModal({ onClose }: PaymentModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (proofType === "ref" && !refNumber.trim()) {
      setError("Ilagay ang GCash reference number.");
      return;
    }
    if (proofType === "image" && !receiptFile) {
      setError("Mag-upload ng larawan ng iyong resibo.");
      return;
    }
    ...
    if (proofType === "image" && receiptFile) {
      ...
      if (uploadError) {
        setError("Hindi na-upload ang resibo: " + uploadError.message);
        setLoading(false);
        return;
      }
    ...
    if (insertError) {
      setError("May error. Subukan ulit o i-message kami direkta.");
    } else {
      setSubmitted(true);
    }
    setLoading(false);
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 px-5 py-5 flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-white text-base">Mag-upgrade ng Listing Slots</h2>
              <p className="text-blue-100 text-xs mt-0.5">+5 slots sa halagang ₱49/buwan</p>
            </div>
          </div>
          ...
          {submitted ? (
            <div className="flex flex-col items-center gap-3 py-4 text-center">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed">
                I-aactivate namin ang iyong +5 slots within 24 hours pagkatapos ma-verify ang iyong bayad.
              </p>
              <button
                onClick={onClose}
                className="mt-2 w-full py-2.5 rounded-xl bg-blue-500 hover:bg-blue-600 text-white text-sm font-semibold transition-colors"
              >
                Sige, salamat!
              </button>
            </div>
            ...
            <>
              ...
              <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-4">
                ...
                <p className="text-sm text-gray-700 dark:text-gray-300">
                  Magpadala ng <strong>₱49</strong> sa GCash number na ito:
                </p>
              ...
              <div>
                ...
                <div className="flex gap-2 mb-4">
                  <button
                    type="button"
                    onClick={() => { setProofType("ref"); setError(""); }}
                  ...
                  <button
                    type="button"
                    onClick={() => { setProofType("image"); setError(""); }}
                    className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border text-sm font-medium transition-colors ${
                      proofType === "image"
                        ? "border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400"
                        : "border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800"
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                  {proofType === "ref" ? (
                    <div>
                      ...
                      <input
                        ...
                        maxLength={30}
                        className="w-full px-3 py-2.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm tracking-widest"
                      />
                      <p className="text-xs text-gray-400 mt-1">
                        Makikita ang reference number sa iyong GCash receipt
                      </p>
                    </div>
                  ) : (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                        Screenshot ng GCash Receipt
                      </label>
                      <div
                        className="border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl p-4 flex flex-col items-center gap-2 cursor-pointer hover:border-blue-400 transition-colors"
                  ...
                  <div className="flex gap-3 mt-1">
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="flex-1 py-2.5 rounded-xl bg-blue-500 hover:bg-blue-600 disabled:opacity-60 text-white text-sm font-semibold transition-colors"
                    >
